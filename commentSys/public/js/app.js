...
function appendToDom(data, flag) {
    ...
    let markup = `     <div class="card border-light mb-3" id="comment-${count}">
                       <div class="card-body ">
  ...
  `
    ...
    if (flag === 1) {
        ...
        //commS.scrollTo(0,commS.scrollHeight);
        ...
        res.json()// stream of data converted to json, will return a promise
            .then((result) => {
                result.forEach((comment) => {
                    ...
                    // flag =0;
                    appendToDom(comment, 0);
                })
                if (bool) {
                    
                    //let lastCBox = document.querySelector(`#comment-${count-1}`);
                    let lastCBox = document.querySelector('#loadMore');
                    window.scrollTo(0,lastCBox.offset );
                    console.log(lastCBox.offsetTop);
                    console.log(count);
...
window.onload = fetchComments(false);